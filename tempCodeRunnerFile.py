...
def update_data():
    ...
    teamsAllValues2=making_trainingset(matches2,players2,teams2)
...
def predictor(teamsCurrStats,teamsAllStats):
    ...
    loaded_model = load_model('autokeras_modelnewestdata.h5', custom_objects=ak.CUSTOM_OBJECTS)

    predictingX=teamsCurrStats[['Rating','openRating','openminRating','openmaxRating','ratingPis','ratingminPis','ratingmaxPis','minRating'
                      ,'maxRating','Kda','minKda',"maxKda",'last10']]
    ...
    print(predsfinal)
    teamsCurrStats.to_excel('preds.xlsx')
...
if(x!='T'):
    ...
    currMatches = today_matches(teams2,players2,matches2)
predictor(currMatches,teamsAllStats1)